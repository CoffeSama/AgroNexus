...
<div class="row">
    ...
    <div class="col-lg-3 col-6">
        <div class="small-box small-box-blue">
            <div class="inner">
                <h3>Bs.{{ number_format($ventas->sum(fn($v) => ($v->cantidad ?? 0) * ($v->preciounitario ?? 0)), 0) }}</h3>
        ...
        <div class="venta-card">
            ...
            <div class="venta-body">
                <div class="venta-info">
                    ...
                    <div class="venta-info-item">
                        <label><i class="fas fa-hashtag mr-1"></i> ID Venta</label>
                        <span>#{{ $v->ventaid }}</span>
                    </div>
                </div>
            </div>
            <div class="venta-footer">
                <small class="text-muted">
                    @if($v->produccion && $v->produccion->fechacosecha)
                        <i class="fas fa-tractor mr-1"></i> Cosechado: {{ \Carbon\Carbon::parse($v->produccion->fechacosecha)->format('d/m/Y') }}
                    @endif
                </small>
                <div>
                    ...
                    <form action="{{ route('ventas.destroy', $v) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar esta venta?')">
                        @csrf
                        @method('DELETE')
                    ...
                    @forelse($ventas as $v)
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <form action="{{ route('ventas.destroy', $v) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar?')">
                                    @csrf @method('DELETE')
