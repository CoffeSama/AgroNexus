                ...
                @forelse($historial as $registro)
                    <div class="historial-item">
                        <div>
                            <div class="fecha">
                                @if($registro->icono)
                                    <img src="https://openweathermap.org/img/wn/{{ $registro->icono }}.png" alt="" style="width: 30px; vertical-align: middle;">
                                @endif
                                {{ $registro->fecha->format('d/m/Y') }}
                                <small class="text-muted">({{ $registro->fecha->diffForHumans() }})</small>
                            </div>
                            <div class="descripcion">{{ $registro->descripcion ?? 'Sin descripción' }}</div>
                        </div>
                        <div class="datos">
                            ...
                            @if($registro->viento)
                                <span class="dato-badge viento">
                                    <i class="fas fa-wind mr-1"></i>{{ $registro->viento }} km/h
                                </span>
                            @endif
                        </div>
                    </div>
...
$(document).ready(function() {
    if (!API_KEY) {
        $('#weather-loading').html('<i class="fas fa-exclamation-triangle fa-3x text-warning"></i><p class="mt-3">API Key no configurada</p>');
    ...
    setInterval(cargarClimaActual, 600000);
});

function cargarClimaActual() {
    $.ajax({
        url: `https://api.openweathermap.org/data/2.5/weather?q=${CIUDAD},${PAIS}&appid=${API_KEY}&units=metric&lang=es`,
        timeout: 15000,
        success: function(data) {
            mostrarClimaActual(data);
        },
        error: function() {
            $('#weather-loading').html('<i class="fas fa-exclamation-triangle fa-3x text-danger"></i><p class="mt-3">Error de conexión</p>');
        }
    });
}
...
function cargarPronostico() {
    $.ajax({
        url: `https://api.openweathermap.org/data/2.5/forecast?q=${CIUDAD},${PAIS}&appid=${API_KEY}&units=metric&lang=es`,
        timeout: 15000,
        success: function(data) {
            mostrarPronostico(data);
        },
        error: function() {
            $('#forecast-container').html('<div class="col text-center text-muted">Error al cargar</div>');
        }
    });
}
...
function mostrarClimaActual(data) {
    ...
    $('#weather-location').text(`${data.name}, Bolivia`);

    $('#detail-humidity').text(data.main.humidity + '%');
    $('#detail-wind').text(Math.round(data.wind.speed * 3.6) + ' km/h');
    $('#detail-pressure').text(data.main.pressure + ' hPa');

    $('#card-temp').text(temp + '°C');
    $('#card-humidity').text(data.main.humidity + '%');
    $('#card-wind').text(Math.round(data.wind.speed * 3.6) + ' km/h');
    $('#card-pressure').text(data.main.pressure + ' hPa');

    const sunrise = new Date(data.sys.sunrise * 1000);
    const sunset = new Date(data.sys.sunset * 1000);
    $('#sunrise').text(sunrise.toLocaleTimeString('es-BO', {hour: '2-digit', minute: '2-digit'}));
    $('#sunset').text(sunset.toLocaleTimeString('es-BO', {hour: '2-digit', minute: '2-digit'}));
...
function mostrarPronostico(data) {
    ...
    data.list.forEach(item => {
        ...
        if (!pronosticoDias[key] || (hora >= 11 && hora <= 14)) {
            pronosticoDias[key] = item;
        }
    });

    const diasArray = Object.values(pronosticoDias).slice(0, 5);
    ...
    diasArray.forEach(item => {
        ...
        html += `
            <div class="col forecast-day">
                <div class="day-name">${dias[fecha.getDay()]}</div>
                <div class="day-date">${fecha.getDate()} ${meses[fecha.getMonth()]}</div>
                <div class="day-icon">
