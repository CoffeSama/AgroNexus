...
<style>
    .catalog-header {
        ...
        margin-bottom: 25px; box-shadow: 0 4px 15px rgba(111, 66, 193, 0.3);
    }
    ...
    .item-card {
        ...
        transition: all 0.3s ease; overflow: hidden; border-left: 4px solid #6f42c1;
    }
    ...
    .item-details p { margin: 5px 0 0; color: #6c757d; font-size: 0.9rem; }
...
<div id="itemsList">
    @forelse ($tipos as $tipo)
        @php
            ...
            $icon = 'tasks';
            foreach ($icons as $key => $val) { if (str_contains(strtolower($tipo->nombre), $key)) { $icon = $val; break; } }
        @endphp
            ...
            <div class="card-body">
                <div class="item-info">
                    ...
                    <div class="item-details">
                        <h5>{{ $tipo->nombre }}</h5>
                        @if($tipo->descripcion)<p>{{ Str::limit($tipo->descripcion, 60) }}</p>@endif
                        <small><i class="fas fa-hashtag mr-1"></i>ID: {{ $tipo->tipoactividadid }}</small>
                    </div>
                </div>
                <div class="item-actions">
                    <a href="{{ route('tipo-actividad.edit', $tipo) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                    <form action="{{ route('tipo-actividad.destroy', $tipo) }}" method="POST" class="d-inline form-eliminar">
                        @csrf @method('DELETE')
    ...
    @empty
        <div class="item-card">
            <div class="empty-state">
                ...
                <a href="{{ route('tipo-actividad.create') }}" class="btn btn-primary"><i class="fas fa-plus mr-2"></i>Agregar Tipo</a>
            </div>
        </div>
    @endforelse
</div>
