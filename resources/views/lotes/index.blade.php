...
@section('title', 'Gestión de Lotes | AgroNexus')
...
<style>
    ...
    .stat-box.produccion { background: linear-gradient(135deg, #388e3c, #66bb6a); }
    .stat-box.produccion h3 { color: white; }
    .stat-box.sembrado { background: linear-gradient(135deg, #0288d1, #4fc3f7); }
    .stat-box.sembrado h3 { color: white; }
    .stat-box.cosechado { background: linear-gradient(135deg, #f57c00, #ffb74d); }
    .stat-box.cosechado h3 { color: white; }
    ...
    .lote-card.en-produccion { border-left-color: #28a745; }
    .lote-card.sembrado { border-left-color: #17a2b8; }
    .lote-card.cosechado { border-left-color: #ffc107; }
    .lote-card.disponible { border-left-color: #6c757d; }
    .lote-card.en-preparacion { border-left-color: #6f42c1; }
...
<div class="row stats-row">
    ...
    <div class="col-md-3 col-6 mb-3">
        <div class="stat-box produccion">
            <h3>{{ $lotes->filter(fn($l) => strtolower($l->estadoTipo->nombre ?? '') == 'en producción')->count() }}</h3>
        ...
        <div class="lote-card {{ $estadoClass }}">
            ...
            <div class="lote-footer">
                ...
                <div>
                    ...
                    <form action="{{ route('lotes.destroy', $l) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este lote?')">
                        @csrf
                        @method('DELETE')
                ...
                <tbody>
                    @forelse($lotes as $l)
                        ...
                        <tr>
                            ...
                            <td><span class="badge {{ $badgeClass }}">{{ ucfirst($l->estadoTipo->nombre ?? 'Sin estado') }}</span></td>
                            <td>{{ $l->superficie }} ha</td>
                            <td>{{ $l->fechasiembra ? \Carbon\Carbon::parse($l->fechasiembra)->format('d/m/Y') : '-' }}</td>
                            <td class="text-center">
                                <a href="{{ route('lotes.show', $l) }}" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a>
                                <a href="{{ route('lotes.edit', $l) }}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                <form action="{{ route('lotes.destroy', $l) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar?')">
                                    @csrf @method('DELETE')
