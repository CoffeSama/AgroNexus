...
<div class="lote-header">
    <div class="row align-items-center">
        <div class="col-md-8">
            ...
            <p class="mb-0 mt-2">
                <i class="fas fa-user mr-1"></i> {{ $lote->usuario->nombre ?? 'Sin asignar' }} {{ $lote->usuario->apellido ?? '' }}
        ...
        <div class="col-md-4 text-md-right">
            ...
            @if($lote->cultivo)
                <br><span class="badge badge-light mt-2" style="font-size: 0.9rem;"><i class="fas fa-seedling mr-1"></i> {{ $lote->cultivo->nombre }}</span>
            @endif
        </div>
    </div>
</div>
        ...
        <div class="tab-content" id="loteTabsContent">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <table class="table info-table">
                            <tr><td>ID</td><td><strong>#{{ $lote->loteid }}</strong></td></tr>
                            <tr><td>Propietario</td><td>{{ $lote->usuario->nombre ?? '-' }} {{ $lote->usuario->apellido ?? '' }}</td></tr>
                            <tr><td>Cultivo</td><td>@if($lote->cultivo)<span class="badge badge-success">{{ $lote->cultivo->nombre }}</span>@else<span class="text-muted">Sin cultivo</span>@endif</td></tr>
                            <tr><td>Estado</td><td><span class="badge {{ $estadoClass }}">{{ ucfirst($lote->estadoTipo->nombre ?? 'Sin estado') }}</span></td></tr>
                            <tr><td>Superficie</td><td><strong>{{ $lote->superficie }}</strong> ha</td></tr>
                            <tr><td>Fecha Siembra</td><td>@if($lote->fechasiembra){{ \Carbon\Carbon::parse($lote->fechasiembra)->format('d/m/Y') }} <small class="text-muted">({{ $estadisticas['dias_desde_siembra'] }} días)</small>@else<span class="text-muted">No registrada</span>@endif</td></tr>
                            <tr><td>Ubicación</td><td>{{ $lote->ubicacion ?? 'No especificada' }}</td></tr>
                            <tr><td>Coordenadas</td><td>@if($lote->latitud && $lote->longitud)<code>{{ $lote->latitud }}, {{ $lote->longitud }}</code>@else<span class="text-muted">No registradas</span>@endif</td></tr>
                        </table>
                    </div>
                    <div class="col-md-6">
                        @if($lote->imagenurl)
                            ...
                            <div class="row">
                                ...
                                <div class="col-6 mb-3">
                                    <div class="p-3 bg-light rounded text-center">
                                        ...
                                        <h4 class="mb-0">{{ $estadisticas['total_actividades'] - $estadisticas['actividades_completadas'] }}</h4>
            ...
            <div class="tab-pane fade" id="trazabilidad" role="tabpanel">
                                ...
                                <div class="timeline-content {{ $evento['color'] }}">
                                    <div class="timeline-date">
                                        ...
                                        @if(isset($evento['completada']))
                                            <span class="badge badge-{{ $evento['completada'] ? 'success' : 'warning' }} ml-2">{{ $evento['completada'] ? 'Completada' : 'Pendiente' }}</span>
                                        @endif
                                    </div>
                ...
                @else
                    <div class="empty-timeline">
                        ...
                        <p class="text-muted">Los eventos aparecerán cuando se registren siembras, insumos, actividades o cosechas.</p>
                    </div>
                @endif
            </div>
            ...
            <div class="tab-pane fade" id="mapa" role="tabpanel">
                @if($lote->latitud && $lote->longitud)
                    <div id="map"></div>
                    <div class="row mt-3">
                        <div class="col-md-6"><div class="p-3 bg-light rounded"><strong><i class="fas fa-map-pin mr-1"></i> Latitud:</strong> {{ $lote->latitud }}</div></div>
                        <div class="col-md-6"><div class="p-3 bg-light rounded"><strong><i class="fas fa-map-pin mr-1"></i> Longitud:</strong> {{ $lote->longitud }}</div></div>
                    </div>
                @else
                    <div class="text-center py-5">
                        ...
                        <a href="{{ route('lotes.edit', $lote) }}" class="btn btn-success"><i class="fas fa-edit mr-1"></i> Agregar</a>
                    </div>
                @endif
            </div>
        </div>
...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-between flex-wrap">
            <a href="{{ route('lotes.index') }}" class="btn btn-secondary btn-action mb-2"><i class="fas fa-arrow-left mr-1"></i> Volver</a>
            <div>
                <a href="{{ route('lotes.edit', $lote) }}" class="btn btn-warning btn-action mb-2"><i class="fas fa-edit mr-1"></i> Editar</a>
