...
<style>
    #map { height: 300px; width: 100%; border-radius: 5px; border: 2px solid #ddd; }
    .image-preview { max-width: 200px; max-height: 150px; border-radius: 5px; border: 2px solid #ddd; }
    .current-image { max-width: 150px; border-radius: 5px; border: 2px solid #28a745; }
    .image-upload-container {
        border: 2px dashed #ccc; border-radius: 5px; padding: 15px;
        text-align: center; cursor: pointer; transition: all 0.3s;
    }
    .image-upload-container:hover { border-color: #28a745; background-color: #f8fff8; }
</style>
@endpush
    ...
    <form action="{{ route('lotes.update', $lote) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label><i class="fas fa-user mr-1"></i> Usuario Propietario <span class="text-danger">*</span></label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nombre" class="form-control" maxlength="100" required value="{{ $lote->nombre }}">
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-map-marker-alt mr-1"></i> Ubicacion (texto)</label>
                        <input type="text" name="ubicacion" class="form-control" maxlength="200" value="{{ $lote->ubicacion }}">
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-ruler-combined mr-1"></i> Superficie (hectareas) <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" name="superficie" id="superficie" class="form-control" min="0" required value="{{ $lote->superficie }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="fechasiembra" class="form-control" value="{{ $lote->fechasiembra ? \Carbon\Carbon::parse($lote->fechasiembra)->format('Y-m-d') : '' }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div id="currentImageContainer" class="mb-2">
                            ...
                            <img src="{{ asset($lote->imagenurl) }}" class="current-image" alt="Imagen actual">
                            <button type="button" class="btn btn-sm btn-outline-danger ml-2" onclick="document.getElementById('currentImageContainer').style.display='none'; document.getElementById('uploadContainer').style.display='block';">
                        ...
                        @endif
                            <div class="image-upload-container" onclick="document.getElementById('imagen').click();">
                                ...
                                <p class="mb-0 small">Clic para seleccionar imagen</p>
                            </div>
                            <input type="file" name="imagen" id="imagen" style="display:none;" accept="image/*" onchange="previewImage(this);">
                            <div id="previewContainer" style="display:none;" class="mt-2">
                                <img id="imagePreview" class="image-preview" alt="Preview">
                                <button type="button" class="btn btn-sm btn-danger ml-2" onclick="removeImage();"><i class="fas fa-times"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label>Latitud</label>
                            <input type="number" step="0.0000001" name="latitud" id="latitud" class="form-control" min="-90" max="90" value="{{ $lote->latitud }}">
                        </div>
                        <div class="form-group col-md-6">
                            <label>Longitud</label>
                            <input type="number" step="0.0000001" name="longitud" id="longitud" class="form-control" min="-180" max="180" value="{{ $lote->longitud }}">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card-footer">
            <div class="d-flex justify-content-between">
                <a href="{{ route('lotes.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left mr-1"></i> Cancelar</a>
